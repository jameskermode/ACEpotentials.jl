...
The `(length(zlist), d)` embedding block for `zlist` (atomic numbers), in that
order, reduced to `d` channels.

`reduction = :pca` (default) works in the principal frame of the selected rows.
With `R = U Σ Vᵀ` (rank `r ≤ length(zlist)`), the coordinates `P = U Σ` carry
the elements' full similarity structure — the cosine Gram of the rows — exactly,
whatever the table's width.  For `d ≤ r` the leading `d` components are kept
(the best rank-`d` approximation); for `d > r`, which is the usual case because
the higher correlation orders need more than `S` channels, the block is `P * Q`
with `Q` an `r × d` matrix with orthonormal rows (DCT-II rows, so it is
deterministic).  `Q Qᵀ = I` keeps the Gram exact, and every column is a generic
direction in `R^S`, which is what the channel-diagonal many-body basis needs to
span `Sym^ν`.

`reduction = :truncate` takes the first `d` columns of the table, which is what the
first spike did.  It is kept for comparison and is **not** a faithful reduction:
measured on CrMnFeCoNi, the cosine Gram of the first 16 of MACE-MP-0-small's 128
channels correlates only 0.52 with the full table's (0.71 for MH-1's 512), and
the first 5 columns are unrelated to it (-0.27 / 0.18).  A model built that way
is using an arbitrary projection, not the foundation model's element similarity.
"""
function embedding_rows(e::ElementEmbedding, zlist; d = size(e.emb, 2),
   ...
   if reduction == :truncate
      rows = e.emb[idx, 1:d]
   elseif reduction == :pca
      rows = _pca_reduce(e.emb[idx, :], d)
   else
      error("reduction = $reduction; expected :pca or :truncate")
   end
   if normalise
      # Normalise AFTER reduction, and this matters far more than it looks.
      ...
      # (Under :pca with d ≥ rank this is a no-op up to rounding: the rows are
      # normalised before the SVD and Q has orthonormal rows.)
      for i = 1:size(rows, 1)
...
# Principal-frame reduction of an (S, D) block to (S, d); see `embedding_rows`.
function _pca_reduce(R::AbstractMatrix, d::Integer)
   R = Matrix{Float64}(R)
   for i = 1:size(R, 1)                      # full-row similarity is the target
      R[i, :] ./= norm(@view R[i, :])
   end
   F = svd(R)
   tol = maximum(size(R)) * eps(Float64) * F.S[1]
   r = count(>(tol), F.S)
   P = F.U[:, 1:r] .* F.S[1:r]'              # S × r, exact Gram
   d <= r && return P[:, 1:d]
   # d > r: mix the r principal coordinates into d generic channels with an
   # r × d matrix of orthonormal rows.  The columns must be GENERIC vectors in
   # R^r: the channel-diagonal many-body basis spans Sym^ν through their ν-th
   # tensor powers, and structured frames fail this -- DCT-II rows, the first
   # choice, reach rank 9 of 15 at order 2 and 13 of 35 at order 3 because
   # products of cosines collapse onto few frequencies.  A pseudo-random frame
   # from an inline xorshift generator is generic and, unlike `Random`'s
   # streams, identical across Julia versions, so a model is reproducible.
   Q = _generic_frame(r, d)
   return P * Q
end

# r × d with orthonormal rows, deterministic, generic.  xorshift64* seeded by
# (r, d) so each shape has its own fixed frame.
function _generic_frame(r::Integer, d::Integer)
   x = UInt64(0x9E3779B97F4A7C15) ⊻ (UInt64(r) << 32) ⊻ UInt64(d)
   Mt = zeros(d, r)
   for j = 1:r, i = 1:d
      x ⊻= x >> 12; x ⊻= x << 25; x ⊻= x >> 27
      Mt[i, j] = 2 * ((x * 0x2545F4914F6CDD1D) >> 11) / 2^53 - 1
   end
...
its coefficients and `acefit!` applies unchanged.  `reduction` selects how the
table is reduced to `d` channels — `:pca` (default, exact element similarity)
or `:truncate` (first `d` columns); see `embedding_rows`.
"""
