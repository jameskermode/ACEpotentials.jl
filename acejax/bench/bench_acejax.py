...
def main():
    ...
    p.add_argument("--no-fold", action="store_true",
                   help="keep the unfolded (B-materialising) readout; the "
                        "baseline row for measuring the fold by difference")
    ...
    model, meta, _ = load(a.npz, dtype=jnp.float32 if a.f32 else jnp.float64,
                          a2b_sparse=a.a2b_sparse, fold=not a.no_fold)
    ...
    print(f"# acejax {'f32' if a.f32 else 'f64'} on {jax.default_backend()}  "
          ...
          f"fold={'no' if a.no_fold else 'yes'}")
