...
#### Spike candidate — Finnis-Sinclair nonlinearity (PACE-style)

**Motivation.** The distilled Cantor-alloy fits sit at ~0.17 eV/A on forces,
which is not production quality, and the obvious lever — polynomial degree — runs
into memory before it runs into diminishing returns (see below). For a *metal*,
a Finnis-Sinclair / EAM-style `sqrt` nonlinearity is the classical answer: it
captures the many-body density term that a linear expansion spends many basis
functions approximating. PACE uses exactly this, `E_i = sum_p c_p (phi_i^(p))^{m_p}`
with `m = 1/2` the important case.

**Status in ACEpotentials v0.10: not implemented, but explicitly hooked.** The
readout is `dot(B, ps.WB[:, iz])`, and three comments mark where it would go:
`src/models/ace.jl:25` ("we can add a nonlinear embedding here"), and `:357` /
`:447` ("here we can insert another nonlinearity instead of the simple dot").
So this is a real implementation task, not a configuration change — and it
touches the evaluation, the gradient and the export.

**The cost, which is the part that matters here.** `E = phi_1 + sqrt(phi_2)` with
`phi_1, phi_2` each linear in their own coefficients is **not** linear in the
parameters jointly, so the fit stops being a convex least-squares problem.
`acefit!`'s QR/LSQR/BLR path does not apply, and BLR's calibrated uncertainty
goes with it. That is precisely the property this project has been protecting.

**A convexity-preserving middle ground worth testing first**, because it is far
cheaper and may capture most of the gain: append `sqrt(rho_i)` as an *extra
basis function*, where `rho_i` is a **fixed** two-body density (not fitted).
The model stays linear in its coefficients — so `acefit!`, the priors and BLR
all still apply — while gaining the dominant EAM-like term. It is strictly less
expressive than full FS, since the embedding function is fixed rather than
learned, but it is a one-column change to the design matrix and it is convex.

**Spike 1 RUN (2026-09-13; `docs/findings/FINDINGS_fs_spike.md`,
`acejax/spike_fs/`).** Fixed `sqrt(rho)` columns, convex, FD-verified to
1e-10. On the bounded Cantor set (200 train / 100 test): the gain is real
(paired-bootstrap CIs exclude zero) and the linear-`rho` control does nothing,
so it is the nonlinearity — but it is modest, and **the classical total
density `sqrt(sum_s rho^s)` (15–30 columns) beats the per-species form (75
columns) 2:1**: test F −5.4% at degree 4, −3.9% at 5, −2.0% at 6; virial
−15–18%. Worth ~3/4 of a degree step at degree 4 and it stacks (degree 5 +
15 columns beat plain degree 6 at that training size). On TiAl DFT: nothing
(< 0.5%), a generalisation-limited fit. **Decision: adopt `sqrt(rho_tot)`
columns as a cheap supplement to degree (a design-matrix change only,
convexity intact); treat full non-convex FS as second-order** — it would
have to buy well over 5% to justify losing BLR/priors/solvers, and the
per-species-vs-total gap says the fitted density is where any extra would
come from.

**Spike 1b — more terms in the embedding function, still linear.** Tested
because GRACE-FS/pacemaker use several densities at several exponents. With
the density FIXED: extra powers (m = 1/8…2), all powers together, a general
Chebyshev `F(rho)` to degree 8 (spline-EAM, linear in its coefficients),
cross-width terms — **at most −1% beyond sqrt at degree 4/5**; `F` saturates
at Chebyshev 4 and over-fits the tails by 8. **The shape of `F` is exhausted
by sqrt.** What is *not* exhausted is the shape of the density: fixed
species-tilted densities `sqrt(rho_tot ± rho^s)` give **−6.8%** at degree 4
(CI [−7.8, −5.7]), 7× any `F`-shape term, and stack with the rest to −12.8%
cumulative (0.1201 → 0.1047, 225 columns, all convex — beating +1 degree and
plain degree 6 at that training size). **So if a fitted FS term is ever
costed, the parameters worth fitting are the species weights inside the
density, not exponents or the embedding polynomial** — and a small set of
fixed tilts already captures the first-order effect convexly.

**Suggested spike order (original):**
1. Fixed `sqrt(rho)` column, convex. Measure against the linear baseline on the
   distilled Cantor data at matched degree. Cheap, and the assembly cache makes
   the comparison nearly free.
2. Only if that is insufficient, full FS with a fitted embedding — which means
   nonlinear least squares and losing the convex guarantees, and should be
   costed as such.

**Compare against raising the degree.** Degree is the alternative lever and it
is measured: at S=5, order 3, parameters go 6 740 (deg 6) -> 19 120 (deg 8) ->
46 635 (deg 10) -> 101 540 (deg 12) categorical, and 1 615 -> 3 765 -> 8 045 ->
15 885 embedded. With in-memory assembly on 62 GB the reachable degree is about
8-10 categorical and 12 embedded; **degree 16-20 needs blocked/streaming
assembly regardless of encoding** (810 GB for an embedded degree-20 design
matrix at 5x observations). If production needs degree 16+, streaming assembly
is the gate on the whole programme -- and a nonlinearity that buys accuracy at
...
- *Distillation into ACE students* is established: Gardner, Thomas du Toit &
  Deringer (arXiv 2506.10956) — MACE-MP → pacemaker ACE, rattle-relax-repeat
  synthetic data, 5–10k structures, ">100x"; and the GRACE foundation-model
  paper (arXiv 2508.17936) — GRACE-2L-OMAT → GRACE-FS on HEA25 (**25
  elements**), 0.56 ms/atom/core, ~70x on CPU, with the finding that
  unary/binary structures in the synthetic set are what keep generalisation.
- *Element embeddings in the radial basis* are Darby et al. (arXiv 2210.01705)
  and are used by MACE and GRACE (GRACE: `R^{μj}_{nl} = Σ_κ w_{kκ} χ_{jκ}(μj) R_nl`,
  128-dim, learned). Pacemaker has none: radials are per element pair.
- *Finnis-Sinclair nonlinearity* is pacemaker's default
  (`FinnisSinclairShiftedScaled`, `[1,1,1,0.5]`, non-linear fit) and GRACE-FS is
  the FS-type fast student GRACE distils into.

So the embedding and the distillation are both well-trodden. **What neither
code has is a tensor-reduced (embedded) ACE fitted linearly and convexly** —
pacemaker is linear but categorical, GRACE is embedded but gradient-descent
only. That is the defensible novelty, and it comes with a caveat measured
above: at S=5 the embedding's values are irrelevant (consistent with Darby's
random-projection result), so "transferred chemistry from the foundation
model" is unproven; it can only show at S > d_max, and GRACE-FS at S=25 is the
comparison to beat. The convex `sqrt(rho)` column is the original 1984
Finnis-Sinclair ansatz on an ACE basis — an unexplored corner, not a new idea.
Extended-distillation (unary/binary structures in the generator) is a recipe
we should adopt before any S>5 run.


**Why now.** Every accuracy statement about element embeddings is blocked on the
same thing: there is no many-element dataset. Distillation removes that block —
MACE is the teacher, so labels are generated rather than found. Three pieces
that did not exist a day ago now make it practical:

- a verified MACE-JAX GPU environment (`/storage/eng/essswb/macejax-gpu/`);
- labelling is **cheap** — measured 16.9 ms per 64-atom frame, so 5 000
  structures is ~85 s of GPU;
- many-element ACE models that build and evaluate at cost flat in S.

And it stays **convex**: the targets change, the model does not, so `acefit!`,
BLR and the committees apply unchanged.

#### The size constraint that sets the experiment

Parameters are `n_B x NZ` — the basis saturates in S but the readout does not:

| S | n_B | parameters | observations needed (~5x) | design matrix |
|---|---|---|---|---|
| 10 | 364 | 3 640 | ~20 000 | ~0.6 GB |
| 20 | 400 | 8 000 | ~40 000 | ~2.6 GB |
...
1. **Structures.** The hard part, and the part that decides whether any of this
   means anything: a teacher can only label what it is shown. Rattled and
   strained prototypes with random element assignment from the pool, with a
   minimum-separation filter so nothing lands where MACE is extrapolating.
   Report the distance and density distributions alongside the errors.
2. **Label** with MACE-MP-0 small on the GPU: energies, forces, virials, into
   extxyz with provenance keys.
3. **Fit** the embedded model with `acefit!`, and — at the same S — a
   categorical `ace1_model` at **matched n_B**, sweeping `lambda` per model.
...
- Embedded within ~10-20% of categorical at matched `n_B` and matched S: the
  embedding is a fair trade, and it buys models categorical cannot build.
- Consistently worse by 2x or more: tensor reduction is too lossy at these
  widths, and `d_max` or the per-order widths need revisiting before anything
  else.
- Student-teacher force RMSE far above the teacher's own error on its training
  distribution: the limit is the structures, not the model. Fix the sampling
  before drawing any conclusion about encodings.
...
- **Incompleteness.** Tensor reduction is known to break the
  ...
  is a separate test and remains unrun.
...
sharing the evaluator's descriptor so fitter and evaluator cannot diverge.
JAX meets all four. The alternatives, assessed: **Rust** (featomic is the
precedent — descriptors with gradients in production) wins on deterministic
CPU performance and deployment but has no AD and no practical GPU path; it
remains a documented option only for a CPU-side, AD-free, ScaLAPACK-adjacent
assembly at very large scale. **C++/Kokkos** (ML-PACE lineage) is the
evaluator's world, not the fitter's. **PyTorch is off the table: MACE itself
is moving to JAX with a full rewrite planned**, so the whole distillation loop
— teacher labelling, `mace-jax` embeddings, student assembly, pre-training,
evaluation — can live in one runtime with one set of array conventions, and
the `mace-jax` coexistence work of Phases 13/15 becomes the intended state
rather than a compatibility chore. **Julia** stays the reference model and
the convex-solve home until the assembly profile (`FINDINGS_assembly_profile`)
says whether its slowness is a fixable instability or structural; on CPU,
allocation-free Julia would match XLA-CPU for these ops, so the GPU is the
decisive JAX advantage, not a language ceiling.
...
| 18. Blocked / streaming / **distributed** assembly so the design matrix need not be resident on one node — **gates three things**, see below | 2–4 |
...
##### Phase 18 in detail: distributed assembly, and why it is now the gate

**What exists today.** `ACEfit/src/assemble.jl` builds `A` and `Y` as
**`SharedArray`s**, parallelising row assembly with `pmap` over packets.
`SharedArray` is *single-node* shared memory (`/dev/shm`), so every worker must
live on one machine and the whole matrix sits in that machine's RAM. Workers
across nodes could not attach. **Multi-node fitting is not implemented.**

**What it gates.** Measured parameter counts for CrMnFeCoNi (S=5, order 3) put
the in-memory reach at roughly **degree 8-10 categorical, degree 12 embedded**
on 62 GB. Beyond that the design matrix exceeds the node, at ~5 observations per
parameter:

| | embedded `d<=16` | categorical |
|---|---|---|
| degree 10 | 6.5 GB | 37.7 GB |
| degree 12 | 12.8 GB | 82 GB (x) |
| degree 16 | 107 GB (x) | — |
| degree 20 | **810 GB (x)** | — |

So this gates **three** things, not one: distillation at many elements,
**polynomial degree 16-20 at any element count**, and any production fit whose
accuracy needs that degree. The distilled Cantor fits sit at ~0.17 eV/A on
forces at degree 6 — not production quality — and degree is the obvious lever.

**The cheap route is an iterative solver, not ScaLAPACK.** `LSQR` already exists
(`solvers.jl:80`, via `IterativeSolvers.jl`) and touches `A` only through `A*x`
and `A'*y`. With rows block-distributed each node holds its own slice, and each
product is a local `mul!` plus one `MPI.Allreduce` of a length-`n_params`
vector — no redistribution, no block-cyclic layout. Two things make it cheap:

- **Rows are already the natural axis.** Observations are independent per
  structure and assembly is already row-parallel over packets, so block-row
  distribution matches how the work is organised. The change is a distributed
  operator type in place of `SharedArray`, not a restructuring.
- **The reduction is tiny.** At degree 20 embedded, `n_params` is 142 250 —
  about 1 MB per Allreduce against a matrix of hundreds of GB.

**MEASURED — see `docs/findings/FINDINGS_scalapack.md`.** A spike compared
ScaLAPACK, Elemental, TSQR, LSQR, Dagger and JAX/lineax on the same
ill-conditioned matrices. Three things above were wrong or overstated:

- **`ScaLAPACK.jl` is dead** — last real commit 2015, does not parse on Julia
  1.11. But **the library works from Julia** via `SCALAPACK32_jll` + `MPI.jl` +
  ~35 lines of `ccall` (`pdgels`, agreement 8e-13 with LAPACK). Kept in reserve.
- **LSQR's `damp` IS QR's `lambda`** for any practical value: solutions agree to
  1e-9..1e-13 for `lambda >= 1e-3` regardless of cond(A). They diverge only as
  `lambda -> 0`, which is `ACEfit.QR()`'s default. The caution above was
  unfounded at the values actually used.
- **"cond ~1e21" overstates real ACE matrices.** That figure came from an
  unregularised design matrix in the embedding spike. With the smoothness prior
  applied, Si_tiny at degree 10-14 is cond **1e11-7e11**, and 3e9-7e9 after
  column equilibration. Still ill-conditioned; not pathological.

**And a limit on TSQR that changes the recommendation.** TSQR (26 lines over
`MPI.jl`, agreement 4e-13..2e-12 to cond 1e21) is the right direct method for
tall-skinny matrices — but its floor is the `n x n` `R` factor: 2 GB at degree 12
embedded, **162 GB at degree 20**. So TSQR covers up to roughly degree 16
embedded; beyond that only LSQR fits a 62 GB node. The seminormal shortcut
(skipping `Q'y`) is catastrophic and must not be used.

**GPU does not change this.** JAX `qr` and `lineax.QR` are numerically
equivalent to LAPACK in f64, but only 1-2x faster at realistic tall-skinny
shapes (A4500 f64 ~280 GFLOP/s) and **OOM at the degree-10 embedded shape** on
20 GB. A 20-line `shard_map` TSQR agrees to 1e-12 on virtual devices; real
multi-GPU was not measured. Not a reason to pull Stage 2A forward.

**Prior art that revises the spike: GAP's `gap_fit`**
([Klawohn, Kermode, Bartók, arXiv:2207.03803](https://arxiv.org/abs/2207.03803)).
Same augmented-QR formulation (`[Σ^{-1/2}K; L^T]`, cf. our `[A; λP]`), solved
with ScaLAPACK on a **`p x 1` process grid — row blocks, one process column** —
which is exactly `ACEfit.assemble`'s layout. Demonstrated to **4 608 cores**:
396 178 x 20 300 (67.6 GB) and 2 482 085 x 20 300 (406 GB), speedups 29x and 39x.

Three things it corrects or sharpens:

- **Routines:** `pdgeqrf` -> `pdormqr` -> `pdtrtrs`, not `pdgels`. With `q = 1`
  the column block size is free; ~100 is optimal for the first two calls and
  it is changed for `pdtrtrs`, which requires equal row and column blocks.
- **The spike's "1-D is 9x slower than 2-D" is suspect.** The paper gets good
  scaling on the 1-D layout with that block-size recipe plus **hybrid
  MPI:OpenMP** (18:4 or 12:6 per 72-core node; "using all cores for MPI results
  in a severe penalty to the solving time"). The spike's 1-D run had neither.
  Re-measure with the paper's settings before choosing between TSQR and
  ScaLAPACK on performance grounds.
- **The identified bottleneck is the solve's inter-node communication**, while
  assembly scales almost perfectly; plus ~0.9-1.5 GB per MPI rank of ScaLAPACK
  work arrays, and a floor of ~2 structures per rank. No GPU.

**And the ratio that decides whether any of this scales for us.** GAP's
observations-to-parameters ratio was **20x** (HEA) and **122x** (SiC); the
distilled Cantor fits run at **5x**. The bound "useful row-block ranks ≈ m/n"
applies to ScaLAPACK-on-`p x 1` exactly as it does to TSQR, so at 5x either
saturates around five fat ranks. The paper is evidence for the row-block
*design* and, equally, that we need far more data per parameter to exploit
hundreds of cores — which distillation can generate cheaply.

**Recommendation adopted:** one row-block distributed operator replacing the
`SharedArray` in `ACEfit.assemble` (~150 lines), with two solvers on top —
**TSQR for degree <= 16 embedded** (direct, QR-equivalent), **LSQR with
mandatory `lambda > 0` beyond** (degree 20, categorical 12). TSQR ~3-4 days,
LSQR +1 day sharing the operator. Elemental (frozen 2017, broken conversions)
and Dagger (10-50x slower) are not recommended. **ScaLAPACK on the same
row-block operator is the proven route for genuine HPC scale** (`gap_fit`,
above) and should be re-benchmarked with the paper's block-size and MPI:OpenMP
recipe before TSQR is preferred over it at more than a handful of nodes.
...
**Redefined (2026-09-13): 2B is basis PRE-TRAINING that feeds 2A, not a
competing fitting mode.** Everything non-linear — the basis — is learned once,
cheaply, on a subset; everything the convex machinery needs — the linear
coefficients — is fitted at scale afterwards, so the *final* model keeps
determinism, BLR/POPS UQ and QP constraints (Stage 2C). This is the
GRACE-FS / PACE function class split into two stages, and it is what the
"keep `Wnlq` live" design decision (§Design decisions 1) was made for.

*What is learned in pre-training:*
- radial mixing `Wnlq` (`LearnableRnlrzzBasis` in Julia; live arrays in acejax);
- the element embedding `E` (S × d) — learned rather than PCA'd from a
  foundation model; the tensor-reduced construction is unchanged;
- the FS densities, framed as linear combinations of the **order-1 basis
  functions**: `rho_i^{(p)} = Σ_k w_k^{(p)} A_k`, i.e. PACE's `φ_2 = Σ c B`.
  Pre-train `w^{(p)}` for a few `p`, freeze, and `sqrt(rho^{(p)})` become
  fixed columns. Spike 1b showed the species weights inside the density are
  the parameters that matter (−6.8% from hand-chosen tilts; the shape of
  `F` is exhausted by sqrt), and this is exactly that family, learned;
- optionally cutoff / transform parameters.

*How:* **variable projection** (Golub–Pereyra), not joint gradient descent.
For fixed non-linear θ the optimal coefficients are the least-squares
solution `c*(θ)`; minimise `‖A(θ) c*(θ) − y‖²` over θ only. With Kaufman's
simplification the θ-gradient is the gradient of the loss **at fixed
`c = c*`** (the `∂c*/∂θ` term drops; same stationary points), so the
algorithm is: assemble `A(θ)`, solve for `c*` (ACEfit — the convex step),
take θ-steps with `c*` frozen, repeat. The outer problem is small (θ ~
thousands, not the ~20k coefficients) and stage two is consistent with it by
construction.

**In Julia, not JAX.** The pieces exist: `grad_params` (site-energy
gradient w.r.t. all `ps`, `Wnlq` included), `pullback_2_mixed` (the mixed
parameter-gradient of force terms, ForwardDiff-over-reverse — the
reverse-over-forward mode the AD section asks for), and an `rrule` for
`energy_forces_virial` w.r.t. `ps`, so `Zygote.gradient` of an E/F/V loss
over the basis parameters works today (it is what the v0.8+ non-linear
tutorial uses). No new AD, no JAX↔Julia round trip, and the θ-gradient comes
from the evaluator that will be exported — the single source of truth
holds. Still to build in Julia: (a) a structured `Wnlq(E)` so the *embedding
table* is the learned object (`set_embedding_weights!` is that map; Zygote
differentiates the indexing), keeping the tensor-reduced structure and the
parameter count; (b) the FS term `sqrt(Σ_k w_k A_k)` inside the model — the
"nonlinear embedding" hook already commented at `ace.jl:25/357/447` — so its
...
radials, the densities are coefficient vectors over existing basis
functions. Assembly, QR/BLR/POPS/QP on the 4k–25k set proceed unchanged.
(The FS columns need adding to the export contract — a small item.)

*Risks, and the checks:* (i) the prize is fewer basis functions at equal
accuracy (MD speed), and a basis optimised on a subset can over-fit it —
hold out within the pre-training set, regularise θ, and confirm transfer
with the learning-curve machinery on the large set; (ii) UQ becomes
conditional on a data-chosen basis, so BLR is slightly over-confident by
construction — POPS covers exactly this. For Stage 2C this is the strongest
form of the comparison: same function class as GRACE-FS, so accuracy at
matched cost should be close, with every convex benefit retained.

**Gate RUN (2026-09-13; `docs/findings/FINDINGS_varpro_spike.md`,
`acejax/spike_fs/varpro_*.jl`) — passes.** Kaufman-VarPro on the FS density
weights (15 per density; ACE block QR'd once, extra columns projected per
iteration at 0.16 s; gradient checked to 5e-7 vs FD and 1e-13 vs
ForwardDiff; monotone throughout). Degree 4, 200/100 Cantor: P=1 learned
density **0.1078** (−10.2% vs linear, −5.7% vs `sqrt(rho_tot)`, 15 columns,
14 s); P=3 **0.1051** (−12.4%), beating the five hand tilts (0.1067, 90
columns) and matching the 225-column convex stack. Learned weights are
interpretable (Mn, Fe up; Ni, Co down; sharper at short range; the data want
α≈7.7). Early stopping changes nothing at this parameter count.
**Transfer holds**: frozen densities + convex refit on a disjoint split give
−9.7/−11.2/−12.1% (in-sample −10.2/−12.0/−12.4%), beat the hand tilts there,
and carry across degree (D4-learned at D5: 0.1040 vs D5 tilts 0.1044).
Caveats: several near-equal optima with different weights (all transfer) —
will matter more for thousands of `Wnlq` parameters; this is a small
non-linear addition to a fixed basis, weaker than a fully data-chosen one.
Next: the same loop over `Wnlq(E)` in Julia via the existing rrules.

*The original 2B brief follows; its concerns about losing convexity apply to
joint fitting and are addressed by the two-stage split above.*
...
#### Stage 2C — the convexity demonstration against GRACE-FS on HEA25S

**Why this milestone exists.** The prior-art check (Stage 1D) settled what is
and is not ours: distillation into ACE students and element embeddings in the
radial basis are both published (Deringer's group; Darby et al.; GRACE), and
the Finnis-Sinclair student is what gracemaker distils *into*. What neither
pacemaker nor gracemaker has is a tensor-reduced (embedded) ACE fitted
**linearly and convexly** — pacemaker is linear but categorical, GRACE is
embedded but gradient-descent only. If convexity is the differentiator, it has
to be *demonstrated*, on the dataset the competition used, in terms that a
gradient-descent model cannot match. "Convex" is not a result; these are:

**What convexity buys, each with a measurable form**

1. **UQ for free, and calibrated.** BLR yields the coefficient posterior from
   the factorisation the fit already did: predictive variance per structure and
   committees sampled from it, no retraining. GRACE-FS gets UQ from a deep
   ensemble (5 seeds = 5x training) or a last-layer approximation.
   *Measure:* calibration (predicted σ vs realised error; z-score histogram,
   ECE) on held-out HEA25S **and** on out-of-distribution structures
   (compositions absent from training, liquid, surfaces), for ACE-BLR vs a
   5-member GRACE-FS ensemble, with the cost of producing each. A clean win if
   calibration matches; a real finding if it does not.

   *And the misspecification-aware version, which is the one that matters for
   a distilled model.* BLR's posterior assumes the model can represent the
   data up to noise; a student that cannot represent its teacher (every
   basis-limited fit in Stage 1D) is **misspecified**, and Bayesian posteriors
   are known to under-estimate parameter uncertainty in exactly that regime.
   Swinburne & Perez's **POPS** (pointwise optimal parameter sets; *Mach.
   Learn.: Sci. Technol.* 2025, arXiv 2402.01810; MLIP application arXiv
   2502.07104; `tomswinburne/POPS-Regression`) is built for it: for each
   training point, the set of parameter perturbations that fit that point
   exactly; a parameter distribution must put mass in every POPS or the
   generalisation error diverges. **It works because the fit is linear** —
   the POPS are evaluated by rank-one updates to a leverage-weighted least
   squares, ~2x the cost of the fit itself, and has been run at
   thousand-parameter MLIP scale. It needs `R` (or `AᵀA`) and the training
   residuals, all of which the factor-once path already holds, and it drops
   straight onto the assembled design matrix. A gradient-descent model has no
   equivalent short of ensembles, which do not target misspecification at all.
   *Measure:* on the same held-out and OOD sets, BLR vs POPS vs GRACE-FS
   ensemble calibration, split by whether the structure is inside the
   teacher's training distribution; the prediction is that BLR is
   over-confident on the basis-limited student and POPS is not.
2. **Optimal data selection, exactly.** The Fisher information is `AᵀA`, so
   D-optimal / leverage-score / MaxVol selection of which structures to label
   is exact and cheap. Distillation is its ideal setting: labels are cheap, fit
   size is the constraint. Pacemaker's extrapolation grade is a MaxVol
   relative, so the argument is one Drautz's group already accepts.
   *Measure:* student error vs number of training structures, selected vs
   random, same teacher. "4k selected ≈ 16k random" is the shape of the claim.
3. **One-shot, predictable, deterministic fit.** Assembly is embarrassingly
   parallel; QR cost is `m n²` on paper; λ comes from closed-form GCV/LOO, not
   a sweep; the result is bit-reproducible with no seeds, epochs or schedules.
   *Measure:* wall time to a converged, hyperparameter-selected model, and
   variance of test error across seeds (ours: zero). Report CPU and GPU
   figures separately — wall clock alone is hardware-confounded.
4. **Exact model selection and delta-learning.** Degree, `d_max`, and a DFT
   correction on top of the distilled fit are convex problems on the same
   basis: distilled-then-DFT-corrected is one more linear solve, not a
   fine-tuning run with a forgetting problem (GRACE's paper spends a section
   on that forgetting).
   *Measure:* accuracy vs DFT after delta-correction with N DFT structures,
   against GRACE's fine-tuning curve (their Fig. 8 shape).
5. **Constrained fits as convex QPs.** Everything of interest is linear in
   the coefficients — energies, forces, virials, hence Hessians and elastic
   constants — so physical requirements become *hard* linear constraints,
   not penalties: equilibrium at the DFT lattice parameter (`A_F c = 0` at
   that geometry), elastic constants and bulk moduli to DFT (equalities),
   phase ordering (`E_A(c) - E_B(c) >= ΔE_DFT`, inequalities), short-range
   repulsion (`dV/dr <= 0` below `r_min`, inequalities), box or L1 bounds.
   Least squares plus any of these is a convex QP with a global optimum;
   with the factor-once `R` the KKT system is `n x n`. A gradient-descent
   model can only weight these, and cannot guarantee any of them. In
   distillation this pins the student to the teacher's (or DFT's) values on
   the unaries and selected binaries exactly and lets least squares do the
   rest. Solver: OSQP.jl / COSMO.jl, or null-space elimination for pure
   equalities within the existing QR path.
   *Measure:* constrained vs unconstrained student — property errors on the
   constrained set (must be ~0), accuracy on everything else (the price of the
   constraints), and the same properties for GRACE-FS with the equivalent
   penalty weights.

**The experiment.** Same data (HEA25S, public), same teacher (GRACE-2L-OMAT,
public — removes the different-teacher confound), same extended-distillation
recipe (unary and binary structures in the synthetic set, which their paper
shows is what preserves generalisation). Student: embedded linear ACE at
S=25, PCA-reduced embedding, plus the convex `sqrt(rho)` column if the spike
justifies it. One table: accuracy vs teacher and vs DFT; MD throughput
(kernel and end-to-end, CPU and GPU); fit wall time and determinism; UQ
calibration and its cost; error vs N under selected vs random labelling;
property errors with and without hard constraints.
GRACE-FS from the paper as the baseline, re-run with gracemaker on identical
splits where possible.

**Gate first: accuracy at matched speed.** S=25 is the first `d_max < S`
regime — order 1 itself is truncated, so the embedding's values finally
matter (at S=5 they did not; that was the 2x2 in Stage 1D). It is not known
whether the linear model reaches GRACE-FS accuracy at a matched MD cost. If
it does not, items 1-5 are wins on a worse model, which is a much weaker
story. So the sequence is:

1. *Accuracy gate* on a 5k-structure HEA25S subset (~1M rows; at ~19k
   parameters a ~150 GB design matrix, which fits `cowf02` today with the
   BIGMEM path). Sweep degree and `d_max`; include the `sqrt(rho)` column.
   Decide on the numbers whether to proceed.
2. *Phase 18* (distributed assembly + TSQR, which still yields `R` for BLR) —
   prerequisite for the full 25k-structure set (~5M rows, ~760 GB).
3. Items 1-5 on the full set.

**Risks.** (i) The accuracy gate fails: linear + fixed `sqrt(rho)` is simply
less expressive than a learned FS embedding at 25 elements. Then the honest
paper is about UQ and data selection at a stated accuracy cost — still
publishable, less compelling. (ii) UQ of a distilled model is relative to the
teacher; calibration vs DFT needs the DFT labels HEA25S has, which is why that
dataset and not a synthetic one. (iii) Reproducing GRACE-FS numbers needs
gracemaker runs on our splits; budget a day for it rather than quoting.
