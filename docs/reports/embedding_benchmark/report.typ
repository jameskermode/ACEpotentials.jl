...
#figure(
  ...
  caption: [Throughput of the degree-6 $d lt.eq 16$ student (638 functions) on one host (moriarty: 32-core Xeon Silver 4216, RTX A4500), 256–384-atom cells, f64 (solid) and f32 (hatched). The JAX kernel runs 16× the teacher in f32 and 52× in f64 through mace-jax; against MACE-torch + cuEquivariance at large cells the f32 figure is 4× (@fig-stacks). The Julia evaluator (CPU, f64 only) reaches 8×10#super[3] atom-steps/s on 32 threads. Neither card is a production GPU: fp64 runs at 1/64 of fp32. The f32 bars use XLA's defaults, i.e. TF32 matmuls on these cards (a 1.2×10#super[−3] descriptor error unless `jax_default_matmul_precision=highest` is set, which acejax's production entry points do); the f64 bars are exact.],
) <fig-thr>

The JAX evaluator's GPU kernel runs 8–17× MACE-MH-1's in f32 at 40–1 300 atoms through mace-jax (3–4× end-to-end with the host neighbour list) and *4.6 × 10#super[4] atom-steps/s in f64* at 330 atoms — 52× MH-1's f64 rate of 8.8 × 10#super[2], because the ACE kernel is gather-bound and barely notices the card's 1/64 fp64 rate while MACE's dense matmuls do. MH-1 does not fit a 3 000-atom cell on 20 GB in either precision. The Julia evaluator is CPU-only (f64): 1.4 × 10#super[3] atom-steps/s on one core of this host and 8.2 × 10#super[3] on 32 threads, i.e. the JAX kernel is 6× the whole node in f64; the categorical degree-6 model (1 348 functions) evaluates 2× faster than the embedded one (638) on CPU because the embedded model's tail blocks carry 7× the AA products — the per-site cost is in the product basis, not the function count, and that is the term Stage 1F's degree rule and the recursive AA products (below) address.
...
#figure(
  ...
  caption: [Kernel throughput in f32 on the A4500 against cell size: the degree-6 embedded $d lt.eq 16$ student (JAX, 638 functions) and MACE-MH-1 / MACE-MP-0 through three stacks — MACE-torch with cuEquivariance, MACE-torch plain, and mace-jax (e3nn-jax). MACE-MP-0 medium (L=1) through all three stacks. cuEquivariance is worth 1.3× → 6.4× to torch as the cell grows, and torch + cuEquivariance overtakes mace-jax above a few hundred atoms.],
) <fig-stacks>

Three things follow (@fig-stacks). cuEquivariance is a large effect at scale — 6.4× on MH-1 and 7.2× on MP-0 medium at ≈1 100 atoms, where torch + cuEquivariance is 1.8× and 2.6× the mace-jax stack — and none at all below ≈100 atoms, where torch dispatch dominates and mace-jax is the fastest MACE (5.3×10#super[3] vs 1.0×10#super[3] on MP-0 medium). The best MACE-MH-1 measured is therefore torch + cuEquivariance at large cells, 1.4×10#super[4] atom-steps/s, still rising with size; against it the student's kernel speed-up is *4×* at ≈1 100 atoms (89× at 40 atoms, 17× at 330), not the 8–17× quoted against mace-jax, and the end-to-end (ASE-frame) ratio there is 1.5×. That is the number to carry into any comparison with a production MACE deployment on a larger card, where cuEquivariance's advantage would grow further. It does not change the accuracy results or the Julia-vs-JAX comparison.
