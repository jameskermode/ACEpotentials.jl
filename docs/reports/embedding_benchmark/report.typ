...
*2A — linear fit at scale, in JAX.* Assembly of the design matrix on the GPU through the same differentiable descriptor the evaluator uses (so fitter and evaluator cannot diverge), and *phase 18*, distributed assembly with a row-block operator feeding TSQR (which still yields $R$ for BLR), ScaLAPACK on the same operator for HPC scale (the `gap_fit` recipe), and LSQR beyond the $R$-factor floor. Sizing: at 4 000 structures the categorical degree-12 matrix is 330 GB and does not fit any single node; the embedded one is 50 GB. The fitter's language is settled as JAX — parameter gradients, GPU assembly (gather-bound, not BLAS-bound), a legible cost model, MACE's own move to JAX — with Julia as the reference model and the home of the convex solve; Rust and PyTorch were assessed and set aside. _Decision sought: that the fitter targets JAX while the model definition stays in Julia, with the CI divergence gate as the contract._
