...
*A correction, stated before the reading.* An earlier version of this report measured the embedded model 7–20% behind the categorical one. That model was mis-built, in three ways found by the tensor-train spike's exact oracle (which requires the embedded model to be a bit-level special case of `ace1_model`): it used solid harmonics where `ace1_model` uses spherical; its pair basis was species-blind and truncated (four pair functions per centre at $S = 5$, none for the fifth species); and its degree rule enumerated 40–45% fewer $(n', l)$ blocks than `ace1_model`'s species-folded rule at the same nominal degree. The fixed model is now an exact special case (at $S = 1$ it *is* `ace1_model`, span residual $5 times 10^(-16)$; at $S = 2$ the lossless model spans the categorical space to $7 times 10^(-15)$), and every embedded number above was re-measured with it. The categorical numbers are unchanged. The embedded parameter counts roughly doubled, because the block set is now the full categorical one.


#figure(
  placement: auto,
  image("fig_accuracy_vs_params.png", width: 92%),
  caption: [Test force RMSE against parameter count (corrected model). Point labels are the polynomial degree. The embedded lossless model tracks or beats the categorical one at every degree with 1.3–1.7× fewer parameters; $d lt.eq 16$ sits within 1–6% at 2.1–3.2× fewer.],
) <fig-acc>
...
#figure(
  ...
  caption: [Learning curves on the 1k set (test solid, train dashed), *measured with the pre-correction embedded model*; the corrected curves are being re-run. Degree 6 is basis-limited for both models; at degree 8 the categorical model is still data-limited (gap 0.022 at N = 800) while the embedded one had converged.],
) <fig-lc>

The learning curves (@fig-lc; pre-correction model, rerun in progress) explain the shape of the table. Both degree-6 models saturate — train and test meet — by 400 structures; the embedded model gets there by 200 and is the better model below that. At degree 8 the categorical model still has slope at 800 and duly gains 11% from 4× the data, while the embedded models gain 2%. *The embedded basis wants degree, not data*; the categorical basis wants data. This is the mechanism behind "accurate per structure vs accurate per parameter".

== The embedding's values, and the ACE1 heuristics (Stage 1F)

A 2×2 of embedding source (MP-0-small, MH-1) × reduction (first-$d$ column truncation, PCA) at degree 6 gave test F within 0.001 eV/Å at $d lt.eq 16$ — *on the pre-correction model*, where the pair-basis and harmonics defects dominated. On the corrected model, with everything else matched (species-symmetric block set, 323 functions, 1k, degree 6), the MH-1 table beats a random projection by 1.8% (0.1063 vs 0.1082): small, as expected at $S = 5$ where $d_max gt.eq S$ leaves order 1 untruncated, but the first correctly-signed sign of transferred chemistry. The MP-0 table itself has no low-rank "chemical" structure (rank 89, 64 components for 98% of the energy), and PCA remains the right reduction (it exposed that a DCT mixing frame spans only 9/15 and 13/35 of the species tensors — a genericity check now in the tests).

The same comparison separates the ACE1 heuristics into what is worth keeping and what is not (all 1k, degree 6, corrected model): the ACE1 species-folded degree rule — which charges different species different radial budgets, an artefact of putting species into $n$ — is worth 7% over a species-symmetric rule (0.0969 vs 0.1038), but only because it admits 2.8× more tail blocks; a symmetric rule at a higher nominal degree gets them without the asymmetry. The ACE1 *radial* heuristics — Jacobi with the envelope folded into the orthogonality, agnesi (2,4), splining, spherical harmonics — are worth 2.3% over Legendre / agnesi (2,2) / unsplined / solid (0.1063 vs 0.1087) and should stay. This is the basis of the Stage 1F proposal below: one model family on `ace_model` with a species-symmetric degree rule, ACE1's radials, and categorical / random / embedded / learned as initialisations of a single radial layer.
