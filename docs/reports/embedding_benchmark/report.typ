...
*None of the MACE numbers in this report use fused equivariant kernels, and they cannot on this stack.* Checked directly: the mace-jax bundles run the plain e3nn-jax path (`equivariance_config: null`); re-loading the same weights with the `cueq` backend enabled routes through `cuequivariance_jax`, but every mace-jax adapter except one calls it with `method='naive'` — the pure-JAX reference implementation, which XLA compiles exactly as it compiles e3nn-jax — so the HLO contains no custom kernels and the "accelerated" model is 2–20% *slower* (layout transposes), at 4×10#super[−9] eV/Å agreement in f64. The one fused kernel (`uniform_1d`, channel-wise tensor product) is reached only with `conv_fusion=True` and needs the CUDA op package and Triton, neither of which the environment had. The 2–5× that cuEquivariance is reported to give MACE is a MACE-torch result. So the speed-ups here are *against the JAX MACE stack as shipped*; a like-for-like accelerated comparison would be against MACE-torch + cuEquivariance, a separate harness, and is the outstanding measurement. The Julia evaluator, however, ran the *embedded* model 1.8× slower than the categorical one despite 4× fewer basis functions, and both far below a well-written CPU kernel. Profiling found the cause: the splined radial tables are LEN wide (224 columns per edge for the embedded model), yet because the embedding weights make every $R_(n l)$ a scalar multiple of $P_(n')$, only 6–8 columns are distinct — the evaluator computes them all, as `SVector{LEN, Dual}`. A prototype exploiting the factorisation ($R_(n l) = c_(n,"pair") P_(n')$, the structure acejax already uses) with an allocation-free site loop is exact to $10^(-12)$ and 12–18× faster on four cores, above the JAX fp64 GPU kernel. On the JAX side the one known size-dependent cost — the reverse-mode scatter behind the edge gather, which degraded the Apple-Silicon MD path 2.3× between 216 and 1 728 atoms — has a measured cure (an algebraically identical one-hot matmul, 5.1× end-to-end with bit-identical trajectories) that is in `acejax` as `edge_a_kind="matmul"` but has not yet been calibrated on the A4500, where the kernel drops 2× between ≈330 and ≈1 100 atoms (Phase 15).
