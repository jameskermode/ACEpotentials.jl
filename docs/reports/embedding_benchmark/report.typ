...
#figure(
  ...
  caption: [Kernel throughput in f32 on the A4500 against cell size: the degree-6 embedded student (JAX) and MACE-MH-1 / MACE-MP-0 through three stacks — MACE-torch with cuEquivariance, MACE-torch plain, and mace-jax (e3nn-jax). The MP-0 torch model is *medium* (L=1); the mace-jax one is *small* (L=0). cuEquivariance is worth 1.3× → 6.4× to torch as the cell grows, and torch + cuEquivariance overtakes mace-jax above a few hundred atoms.],
) <fig-stacks>

Three things follow (@fig-stacks). cuEquivariance is a large effect at scale — 6.4× on MH-1 at ≈1 100 atoms — and none at all below ≈100 atoms, where torch dispatch dominates and mace-jax is the fastest MACE. The best MACE-MH-1 measured is therefore torch + cuEquivariance at large cells, 1.4×10#super[4] atom-steps/s, still rising with size; against it the student's kernel speed-up is *6×* at ≈1 100 atoms (109× at 40 atoms, 25× at 330), not the 11–38× quoted against mace-jax, and the end-to-end (ASE-frame) ratio there is 3×. That is the number to carry into any comparison with a production MACE deployment on a larger card, where cuEquivariance's advantage would grow further. It does not change the accuracy results or the Julia-vs-JAX comparison.
